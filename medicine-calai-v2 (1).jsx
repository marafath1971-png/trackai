...
function CelebrationModal({med,onDone}) {
  const L=useTheme();
  const isCourse = med.type === "course";
  const isAllDone = !isCourse;
  const emoji = isCourse ? "🏆" : "🎉";
  const title = isCourse ? "Course Complete!" : "All Done Today!";
  const subtitle = isCourse ? med.message||"You finished your full medicine course! Excellent adherence!" : "You took every dose today. Keep the streak going! 🔥";
  const btnColor = isCourse ? L.green : "#111";
  const ff="'Figtree',-apple-system,'Helvetica Neue',sans-serif";
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(24px) saturate(160%)",WebkitBackdropFilter:"blur(24px) saturate(160%)"}}>
      <Confetti/>
      <div style={{background:L.card,borderRadius:24,padding:"32px 28px",textAlign:"center",maxWidth:320,width:"100%",animation:"celebPop 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards",position:"relative",zIndex:1001,border:`2px solid ${isCourse?L.green:"#111"}20`,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        <div style={{fontSize:72,marginBottom:8,animation:"floatUp 2s ease-in-out infinite"}}>{emoji}</div>
        <h2 style={{fontFamily:ff,fontSize:26,fontWeight:800,color:L.text,margin:"0 0 8px",lineHeight:1.2,letterSpacing:"-0.5px"}}>{title}</h2>
        {isCourse&&<p style={{color:btnColor,fontWeight:700,fontSize:16,margin:"0 0 8px",fontFamily:ff}}>{med.name}</p>}
        <p style={{color:L.sub,fontSize:14,lineHeight:1.65,margin:"0 0 24px"}}>{subtitle}</p>
        {isCourse&&(
          <div style={{background:"#F0FDF4",borderRadius:14,padding:"12px 16px",marginBottom:20,border:"1px solid #BBF7D0"}}>
            <p style={{margin:0,fontSize:13,fontWeight:700,color:L.green}}>✓ Full course completed</p>
            <p style={{margin:"3px 0 0",fontSize:12,color:"#16A34A"}}>Talk to your doctor before stopping any medication</p>
          </div>
        )}
        <button onClick={onDone} style={{width:"100%",padding:"15px",background:btnColor,border:"none",borderRadius:14,fontSize:16,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:ff}}>
          {isCourse?"Excellent! Done ✓":"Keep it up! ✓"}
        </button>
      </div>
    </div>
  );
...
function AppShell({children,tab,setTab,notifCount,missedAlertCount,darkMode=false,onScanFAB}) {
  ...
  const navItems=[
    ...
    {id:"dashboard", label:"Dashboard", icon:"M18 20V10 M12 20V4 M6 20v-6"},
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    meds.forEach(m=>{
      if(!m.courseCompleted){
        const isFixedComplete = m.courseType==="fixed" && m.courseEndDate && today>=m.courseEndDate;
        const isPillsGone = m.courseType==="fixed" && m.count===0 && m.totalCount>0;
        if(isFixedComplete||isPillsGone){
          updateMed(m.id,{courseCompleted:true});
          setTimeout(()=>setCelebration({name:m.name,color:m.color,type:"course",message:"Course complete! 🎓"}),500);
        }
      }
    });
  },[meds,phase]);
  ...
  const saveScan=useCallback((r)=>{
    ...
    // Build auto-schedule from AI-extracted suggestedSchedule
    const defaultSched = r.suggestedSchedule && r.suggestedSchedule.length > 0
      ? r.suggestedSchedule.map((s,i)=>({
          label:s.label||["Morning","Afternoon","Evening","Bedtime"][i]||"Dose "+(i+1),
          time:{h:s.hour||8,m:s.minute||0},
          days:[0,1,2,3,4,5,6],
          enabled:true,
          withFood:s.withFood||r.withFood||false,
        }))
      : [{label:"Morning",time:{h:8,m:0},days:[0,1,2,3,4,5,6],enabled:true,withFood:r.withFood||false}];
    const courseDays = r.courseDurationDays||0;
    const courseEnd = courseDays>0 ? (() => { const d=new Date(); d.setDate(d.getDate()+courseDays); return d.toISOString().slice(0,10); })() : null;
    setMeds(p=>[...p,{
      ...
      refillAt:r.refillAlert||7,imageUrl:r.imageUrl,
      notes:r.description||"",howToTake:r.howToTake||"",
      warnings:r.warnings||"",withFood:r.withFood||false,
      schedule:defaultSched,courseStartDate:todayStr(),
      courseDurationDays:courseDays,courseEndDate:courseEnd,
      courseType:r.courseType||"ongoing",
      courseCompleted:false,
      isLiquid:isLiq,volumeUnit:r.volumeUnit||"ml",dosePerTake:r.dosePerTake||"",
    }]);
    setScanState("idle"); setScanResult(null); setTab("home");
    showToast((r.name||"Medicine")+" added with schedule ✓","success");
  },[showToast]);

  async function doScan(file){
    try{
      const b64=await f2b64(file), imgUrl=URL.createObjectURL(file);
      const prompt=`You are a clinical pharmacist AI. Analyse this medicine image extremely carefully.
Determine if SOLID (tablet/capsule/pill) or LIQUID (syrup/suspension/drops/solution/cream/gel/inhaler/injection).
Extract ALL visible details including dosing schedule, course duration, and instructions.
Respond ONLY with valid JSON (no markdown, no explanation):
{
  ...
  "isLiquid": false,
  "category": "e.g. Antibiotic|Painkiller|Diabetes|Blood Pressure|Antihistamine|Vitamin",
  "description": "what it treats in 1 clear sentence",
  "howToTake": "detailed instructions e.g. Take 1 tablet twice daily after meals",
  "withFood": true,
  "sideEffects": "top 2-3 side effects comma separated",
  "interactions": "key drug interactions",
  "storage": "storage instructions",
  "warnings": "important warnings e.g. Do not crush, Avoid alcohol",
  "pillCount": 30,
  "packSize": 30,
  "refillAlert": 7,
  "volumeAmount": 0,
  ...
  "courseType": "ongoing|fixed|as-needed",
  "suggestedSchedule": [
    {"label": "Morning", "hour": 8, "minute": 0, "withFood": true},
    {"label": "Evening", "hour": 20, "minute": 0, "withFood": true}
  ],
  "confidence": "high|medium|low"
}
Rules:
- LIQUID (syrup, drops, cream, gel): isLiquid=true, volumeAmount=total ml, dosePerTake="5ml", pillCount=0
- SOLID (tablet, capsule): isLiquid=false, pillCount=count visible or standard pack
- INHALERS: isLiquid=true, volumeUnit="puffs", volumeAmount=200
- courseDurationDays: extract from label (e.g. "7 days"=7, "complete course"=7, "ongoing"=0)
- courseType: "fixed" for antibiotics (5-14 days), "ongoing" for chronic (BP/diabetes), "as-needed" for PRN
- dosesPerDay: infer (once=1, twice=2, three times=3, four times=4)
- suggestedSchedule: create time slots based on dosesPerDay using typical meal times
- withFood: true if packaging says "with meals/food/after eating"
- If no medicine detected: set identified=false`;
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:MODEL,max_tokens:900,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:file.type||"image/jpeg",data:b64}},{type:"text",text:prompt}]}]})});
  ...
  return(
    ...
    <AppShell tab={tab} setTab={setTab} notifCount={notifQueue.length} missedAlertCount={missedAlerts.filter(a=>!a.seen).length} darkMode={darkMode}>
      ...
      {tab==="scan"&&<ScanTab showToast={showToast} state={scanState} result={scanResult} camRef={camRef} galRef={galRef} onFile={doScan} onReset={()=>{setScanState("idle");setScanResult(null);}} onSave={(r)=>{saveScan(r);}} setScanResult={setScanResult} setScanState={setScanState}/>}
      {tab==="dashboard"&&<DashboardTab meds={meds} doses={doses} takenToday={takenToday} takenCount={takenCount} dosePct={dosePct} history={history} today={today} streak={streak} onToggle={handleToggleDose} onDetail={setDetailMed} lowMeds={lowMeds} profile={profile} setTab={setTab}/>}
          ...
          <div style={{background:L.card,borderRadius:14,padding:18,boxShadow:"inset 0 -0.5px 0 rgba(60,60,67,0.2)",marginBottom:12}}>
            {result.identified?(
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                {result.withFood&&<IRow label="With food" value="✓ Take with meals"/>}
                {result.warnings&&<IRow label="⚠️ Warning" value={result.warnings}/>}
                {result.sideEffects&&<IRow label="Side effects" value={result.sideEffects}/>}
                {result.courseDurationDays>0&&<IRow label="Course" value={`${result.courseDurationDays} days (${result.courseType})`}/>}
                {result.suggestedSchedule&&result.suggestedSchedule.length>0&&<IRow label="Schedule" value={result.suggestedSchedule.map(s=>s.label).join(", ")+" (auto-set)"}/>}
...
/* ══════════════════════════════════════════════
   DASHBOARD TAB — Smart Medicine Overview
══════════════════════════════════════════════ */
function DashboardTab({meds,doses,takenToday,takenCount,dosePct,history,today,streak,onToggle,onDetail,lowMeds,profile,setTab}) {
  const L=useTheme();
  const ff="'Figtree',-apple-system,'Helvetica Neue',sans-serif";
  ...
  const missedDoses=doses.filter(d=>{
    const now=new Date();
    const overdue=(d.sched.time.h*60+d.sched.time.m)<(now.getHours()*60+now.getMinutes());
    return !takenToday[d.key] && overdue;
  }).length;

  // Active courses (fixed duration meds)
  const activeCourses=meds.filter(m=>m.courseType==="fixed"&&!m.courseCompleted&&m.courseEndDate);
  const completedCourses=meds.filter(m=>m.courseCompleted).length;

  // Weekly adherence for bar chart
  const weekDays=Array.from({length:7},(_,i)=>{
    const d=new Date(); d.setDate(d.getDate()-(6-i));
    const k=d.toISOString().slice(0,10);
    const ds=history[k]||[];
    const rate=ds.length?ds.filter(e=>e.taken).length/ds.length:0;
    return {k,rate,label:["S","M","T","W","T","F","S"][d.getDay()],isT:k===today};
  });
  ...
  const last30Entries=last30.flatMap(k=>history[k]||[]);
  const overallAdh=last30Entries.length?Math.round(last30Entries.filter(e=>e.taken).length/last30Entries.length*100):0;

  return(
    <div style={{padding:"0 20px",paddingBottom:40}}>
      <div style={{paddingTop:60,paddingBottom:8}}>
        <h1 style={{fontFamily:ff,fontSize:28,fontWeight:800,margin:"0 0 2px",color:L.text,letterSpacing:"-0.8px"}}>Dashboard</h1>
        <p style={{color:L.sub,fontSize:13,margin:0}}>Your complete medicine overview</p>
      </div>

      {/* Hero stats row */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
        {[
          {emoji:"✅",val:`${Math.round(dosePct*100)}%`,label:"Today",color:dosePct===1?L.green:dosePct>0.5?L.amber:L.red},
          {emoji:"📈",val:`${overallAdh}%`,label:"30-Day",color:overallAdh>=80?L.green:overallAdh>50?L.amber:L.red},
          {emoji:"🔥",val:streak,label:"Streak",color:streak>7?"#F97316":L.text},
        ].map((s,i)=>(
          <div key={i} style={{background:L.card,borderRadius:16,padding:"14px 12px",textAlign:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
            <span style={{fontSize:20}}>{s.emoji}</span>
            <p style={{margin:"6px 0 2px",fontSize:22,fontWeight:800,color:s.color,letterSpacing:"-0.8px",fontFamily:ff}}>{s.val}</p>
            <p style={{margin:0,fontSize:11,color:L.sub,fontWeight:600}}>{s.label}</p>
          </div>
        ))}
      </div>

      {/* Today's progress card */}
      <div style={{background:L.card,borderRadius:18,padding:"18px 18px",marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div>
            <p style={{margin:0,fontWeight:800,fontSize:16,color:L.text,fontFamily:ff}}>Today's Schedule</p>
            <p style={{margin:"2px 0 0",fontSize:12,color:L.sub}}>{takenCount}/{totalDoses} doses taken{missedDoses>0?` · ${missedDoses} missed`:""}</p>
          </div>
          <div style={{width:56,height:56}}>
            <svg width={56} height={56} style={{transform:"rotate(-90deg)"}}>
              <circle cx={28} cy={28} r={22} fill="none" stroke="rgba(120,120,128,0.12)" strokeWidth={7}/>
              <circle cx={28} cy={28} r={22} fill="none" stroke={dosePct===1?L.green:dosePct>0.5?L.amber:L.blue} strokeWidth={7}
                strokeLinecap="round" strokeDasharray={`${2*Math.PI*22*Math.min(1,dosePct)} ${2*Math.PI*22}`} style={{transition:"stroke-dasharray 0.7s"}}/>
            </svg>
          </div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {doses.length===0?(
            <div style={{textAlign:"center",padding:"20px 0"}}>
              <p style={{color:L.sub,fontSize:13,margin:0}}>No doses scheduled today</p>
              <button onClick={()=>setTab("scan")} style={{marginTop:10,padding:"8px 18px",background:"#111",border:"none",borderRadius:99,fontSize:12,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:ff}}>+ Scan Medicine</button>
            </div>
          ):doses.map(d=>{
            const taken=takenToday[d.key];
            const now=new Date();
            const overdue=!taken&&(d.sched.time.h*60+d.sched.time.m)<(now.getHours()*60+now.getMinutes());
            return(
              <div key={d.key} onClick={()=>onToggle(d.key)}
                style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:12,background:taken?"#F0FDF4":overdue?"#FFF5F5":L.bg,border:`1px solid ${taken?"#BBF7D0":overdue?"#FCA5A5":L.border}`,cursor:"pointer",transition:"all 0.2s"}}>
                <div style={{width:36,height:36,borderRadius:10,background:taken?"#DCFCE7":`${d.med.color}18`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <span style={{fontSize:18}}>{d.med.isLiquid?"🧴":"💊"}</span>
                </div>
                <div style={{flex:1}}>
                  <p style={{margin:0,fontWeight:700,fontSize:13,color:taken?"rgba(0,0,0,0.4)":L.text,textDecoration:taken?"line-through":"none",fontFamily:ff}}>{d.med.name}</p>
                  <p style={{margin:"1px 0 0",fontSize:11,color:L.sub}}>{d.med.dose}{d.sched.withFood?" · With food":""}  · {fmt(d.sched.time.h,d.sched.time.m)}</p>
                </div>
                <div style={{width:24,height:24,borderRadius:99,background:taken?"#111":overdue?"#FEE2E2":L.fill,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.2s"}}>
                  {taken&&<Ic d={ic.check} size={11} c="#fff" w={2.8}/>}
                  {overdue&&!taken&&<span style={{fontSize:10}}>!</span>}
                </div>
              </div>
            );
          })}
      ...
      <div style={{background:L.card,borderRadius:18,padding:"16px 18px",marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        <p style={{margin:"0 0 14px",fontWeight:800,fontSize:15,color:L.text,fontFamily:ff}}>This Week</p>
        <div style={{display:"flex",alignItems:"flex-end",gap:6,height:70}}>
          {weekDays.map((w,i)=>(
            <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
              <div style={{width:"100%",height:52,background:L.border,borderRadius:8,overflow:"hidden",position:"relative"}}>
                <div style={{position:"absolute",bottom:0,width:"100%",borderRadius:8,
                  height:w.isT?`${Math.max(12,dosePct*52)}px`:w.rate>0?`${Math.max(6,w.rate*52)}px`:"3px",
                  background:w.isT?(dosePct>=1?L.green:L.blue):w.rate>=0.8?L.green:w.rate>0?L.amber:L.border,
                  transition:"height 0.5s cubic-bezier(0.4,0,0.2,1)"}}/>
              </div>
              <span style={{fontSize:10,fontWeight:700,color:w.isT?L.blue:L.sub}}>{w.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {activeCourses.length>0&&(
        <div style={{background:L.card,borderRadius:18,padding:"16px 18px",marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          <p style={{margin:"0 0 12px",fontWeight:800,fontSize:15,color:L.text,fontFamily:ff}}>Active Courses</p>
          {activeCourses.map(m=>{
            const start=new Date(m.courseStartDate+"T12:00:00");
            const end=new Date(m.courseEndDate+"T12:00:00");
            const total=Math.round((end-start)/(1000*60*60*24));
            const done=Math.round((new Date()-start)/(1000*60*60*24));
            const pct=Math.min(1,Math.max(0,done/total));
            const daysLeft=Math.max(0,Math.round((end-new Date())/(1000*60*60*24)));
            return(
              <div key={m.id} onClick={()=>onDetail(m)} style={{marginBottom:10,padding:"12px 14px",background:L.bg,borderRadius:14,cursor:"pointer",border:`1px solid ${L.border}`}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{width:10,height:10,borderRadius:99,background:m.color}}/>
                    <p style={{margin:0,fontWeight:700,fontSize:14,color:L.text,fontFamily:ff}}>{m.name}</p>
                    <span style={{fontSize:10,fontWeight:700,color:"#fff",background:m.color,padding:"2px 8px",borderRadius:99}}>{m.dose}</span>
                  </div>
                  <span style={{fontSize:11,fontWeight:700,color:daysLeft<=2?L.red:L.sub}}>{daysLeft} day{daysLeft!==1?"s":""} left</span>
                </div>
                <div style={{height:6,background:L.border,borderRadius:99,overflow:"hidden",marginBottom:4}}>
                  <div style={{height:"100%",width:`${pct*100}%`,background:m.color,borderRadius:99,transition:"width 0.5s"}}/>
                </div>
                <p style={{margin:0,fontSize:11,color:L.sub}}>Day {Math.min(done+1,total)} of {total} · {m.category}</p>
              </div>
            );
          })}
        </div>
      ...
      {meds.length>0&&(
        <div style={{background:L.card,borderRadius:18,padding:"16px 18px",marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          <p style={{margin:"0 0 12px",fontWeight:800,fontSize:15,color:L.text,fontFamily:ff}}>Medicine Stock</p>
          {meds.map((m,i)=>{
            const pct=Math.max(0,Math.min(1,m.count/m.totalCount));
            const isLow=m.count<=m.refillAt;
            const unit=m.isLiquid?(m.volumeUnit||"ml"):"";
            return(
              <div key={m.id} onClick={()=>onDetail(m)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:i<meds.length-1?`1px solid ${L.border}`:"none",cursor:"pointer"}}>
                <div style={{width:10,height:10,borderRadius:99,background:isLow?L.red:m.color,flexShrink:0}}/>
                <div style={{flex:1}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <p style={{margin:0,fontWeight:700,fontSize:13,color:L.text,fontFamily:ff}}>{m.name}</p>
                    <span style={{fontSize:11,fontWeight:700,color:isLow?L.red:L.sub}}>{m.count}{unit}{isLow?" ⚠️":""}</span>
                  </div>
                  <div style={{height:4,background:L.border,borderRadius:99,overflow:"hidden"}}>
                    <div style={{width:`${pct*100}%`,height:"100%",background:isLow?L.red:m.color,borderRadius:99,transition:"width 0.4s"}}/>
                  </div>
                </div>
              </div>
            );
      ...
      {/* Quick instructions for active meds */}
      {meds.filter(m=>m.howToTake||m.withFood||m.warnings).length>0&&(
        <div style={{background:L.card,borderRadius:18,padding:"16px 18px",marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          <p style={{margin:"0 0 12px",fontWeight:800,fontSize:15,color:L.text,fontFamily:ff}}>Instructions</p>
          {meds.filter(m=>m.howToTake||m.withFood||m.warnings).map((m,i,arr)=>(
            <div key={m.id} style={{paddingBottom:i<arr.length-1?12:0,marginBottom:i<arr.length-1?12:0,borderBottom:i<arr.length-1?`1px solid ${L.border}`:"none"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                <div style={{width:8,height:8,borderRadius:99,background:m.color}}/>
                <p style={{margin:0,fontWeight:700,fontSize:13,color:L.text,fontFamily:ff}}>{m.name}</p>
              </div>
              {m.howToTake&&<p style={{margin:"0 0 3px",fontSize:12,color:L.sub,paddingLeft:14}}>💊 {m.howToTake}</p>}
              {m.withFood&&<p style={{margin:"0 0 3px",fontSize:12,color:"#92400E",paddingLeft:14}}>🍽️ Take with food</p>}
              {m.warnings&&<p style={{margin:0,fontSize:12,color:L.red,paddingLeft:14}}>⚠️ {m.warnings}</p>}
            </div>
          ))}
        </div>
      )}
      ...
      {/* Empty state */}
      {meds.length===0&&(
        <div style={{textAlign:"center",padding:"50px 20px",background:L.card,borderRadius:18,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          <span style={{fontSize:56,display:"block",marginBottom:14}}>📊</span>
          <p style={{fontWeight:800,fontSize:18,color:L.text,margin:"0 0 6px",fontFamily:ff}}>No medicines yet</p>
          <p style={{color:L.sub,fontSize:13,margin:"0 0 18px",lineHeight:1.5}}>Scan your first medicine to unlock your personal dashboard</p>
          <button onClick={()=>setTab("scan")} style={{padding:"13px 28px",background:"#111",border:"none",borderRadius:99,fontSize:14,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:ff}}>📷 Scan Medicine</button>
        </div>
      )}
    </div>
  );

...
class UserRepositoryImpl implements IUserRepository {
  ...
  Future<UserProfile?> getProfile() async {
    if (_hasAuth) {
      final cloud = await firestoreDataSource
          .getProfile(_uid!)
          .withHardenedTimeout(taskName: 'getProfile');
      if (cloud != null) {
  ...
  Future<UserProfile?> getOtherProfile(String uid) async {
    return await firestoreDataSource
        .getProfile(uid)
        .withHardenedTimeout(taskName: 'getOtherProfile');
  }
  ...
  Future<List<Caregiver>> getCaregivers() async {
    if (_hasAuth) {
      final cloud = await firestoreDataSource
          .getCaregivers(_uid!)
          .withHardenedTimeout(taskName: 'getCaregivers');
      if (cloud.isNotEmpty) {
  ...
  Future<StreakData> getStreakData() async {
    if (_hasAuth) {
      final cloud = await firestoreDataSource
          .getStreakData(_uid!)
          .withHardenedTimeout(taskName: 'getStreakData');
  ...
  Future<bool> getDarkMode() async {
    if (_hasAuth) {
      return firestoreDataSource
          .getDarkMode(_uid!)
          .withHardenedTimeout(taskName: 'getDarkMode');
    }
  ...
  Future<String> getLanguage() async {
    if (_hasAuth) {
      final cloud = await firestoreDataSource
          .getLanguage(_uid!)
          .withHardenedTimeout(taskName: 'getLanguage');
      if (cloud != null) {

...
class UserRepositoryImpl implements IUserRepository {
  ...
  Future<UserProfile?> getProfile() async {
    if (_hasAuth) {
      final cloud = await firestoreDataSource.getProfile(_uid!);
      if (cloud != null) {
  ...
  Future<void> saveProfile(UserProfile profile) async {
    ...
    if (_hasAuth) {
      firestoreDataSource.saveProfile(_uid!, profile).catchError((_) {});
    }
  }
  ...
  Future<List<Caregiver>> getCaregivers() async {
    if (_hasAuth) {
      final cloud = await firestoreDataSource.getCaregivers(_uid!);
      if (cloud.isNotEmpty) {
  ...
  Future<void> saveCaregivers(List<Caregiver> caregivers) async {
    await localDataSource.setJson(
        ...
        encrypt: true);
    if (_hasAuth) {
      firestoreDataSource.saveCaregivers(_uid!, caregivers).catchError((_) {});
    }
  }
  ...
  Future<StreakData> getStreakData() async {
    if (_hasAuth) {
      final cloud = await firestoreDataSource.getStreakData(_uid!);
  ...
  Future<void> saveStreakData(StreakData data) async {
    ...
    if (_hasAuth) {
      firestoreDataSource.saveStreakData(_uid!, data).catchError((_) {});
    }
  }
  ...
  Future<bool> getDarkMode() async {
    if (_hasAuth) {
      return firestoreDataSource.getDarkMode(_uid!);
    }
